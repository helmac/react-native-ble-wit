...
class Peripheral:Hashable {
    ...
    func setSendCharacteristic(sendCharacteristic:CBCharacteristic){
        self.sendCharacteristic = sendCharacteristic
        self.ble?.setSendCharacteristic(sendCharacteristic: sendCharacteristic)
    }
    ...
    func getDeviceData(_ key: String) -> String? {
        return self.bwt901ble?.getDeviceData(key)
    }
    ...
    func toBluetoothBLE() -> BluetoothBLE {
        let peripheral = instance
        
        return BluetoothBLE(peripheral, self.sendCharacteristic)
    }
    
    func invokeDataRecevied(data:[UInt8]){
        self.ble?.invokeDataRecevied(data: data)
    }
}
