...
extension DeviceModel{
    ...
    // MARK: 打开设备
    func openDevice(ble:BluetoothBLE) throws{
        
        if (coreConnect != nil) {
            // 打开连接器
            try coreConnect?.open(ble: ble)
    ...
    // MARK: 重新打开
    func reOpen(ble:BluetoothBLE) throws{
        self.closeDevice()
        try self.openDevice(ble: ble)
    }
...
extension DeviceModel {
    ...
    func invokeKeyUpdateObserver(_ deviceModel:DeviceModel, _ key:String,_ value:String){
        NSLog("invokeKeyUpdateObserver -> \(key) =")
