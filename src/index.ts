...
let witBleManager = NativeModules.WitBleManager;

class WitBleManager extends NativeEventEmitter {
  constructor() {
    super(witBleManager);
  ...
  retrieveServices(peripheralId: string, serviceUUIDs: string[] = []) {
    return new Promise<PeripheralInfo>((fulfill, reject) => {
      witBleManager.retrieveServices(
        peripheralId,
        serviceUUIDs,
   ...
   * @returns
   */
  getData(peripheralId: string) {
    return new Promise<PeripheralData>((fulfill, reject) => {
      witBleManager.getData(
        peripheralId,
        (error: string | null, data: PeripheralData) => {
          if (error) {
            reject(error);
          } else {
            fulfill(data);
  ...
  writeWithoutResponse(
    ...
    return new Promise<void>((fulfill, reject) => {
      witBleManager.writeWithoutResponse(
        peripheralId,
        serviceUUID,
        characteristicUUID,
        data,
  ...
  connect(peripheralId: string, options?: ConnectOptions) {
    return new Promise<void>((fulfill, reject) => {
      ...
      witBleManager.connect(peripheralId, options, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  removeBond(peripheralId: string) {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.removeBond(peripheralId, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  disconnect(peripheralId: string, force: boolean = true) {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.disconnect(peripheralId, force, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  startNotification(
    ...
    return new Promise<void>((fulfill, reject) => {
      witBleManager.startNotification(
        peripheralId,
        serviceUUID,
        characteristicUUID,
        (error: string | null) => {
  ...
  startNotificationUseBuffer(
    ...
    return new Promise<void>((fulfill, reject) => {
      witBleManager.startNotificationUseBuffer(
        peripheralId,
        serviceUUID,
        characteristicUUID,
        buffer,
  ...
  stopNotification(
    ...
    return new Promise<void>((fulfill, reject) => {
      witBleManager.stopNotification(
        peripheralId,
        serviceUUID,
        characteristicUUID,
        (error: string | null) => {
  ...
  start(options?: StartOptions) {
    return new Promise<void>((fulfill, reject) => {
      ...
      witBleManager.start(options, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  stopScan() {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.stopScan((error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  enableBluetooth() {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.enableBluetooth((error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  getConnectedPeripherals(serviceUUIDs: string[] = []) {
    return new Promise<Peripheral[]>((fulfill, reject) => {
      witBleManager.getConnectedPeripherals(
        serviceUUIDs,
  ...
  getBondedPeripherals() {
    return new Promise<Peripheral[]>((fulfill, reject) => {
      witBleManager.getBondedPeripherals(
        (error: string | null, result: Peripheral[] | null) => {
          if (error) {
            reject(error);
          } else {
  ...
  getDiscoveredPeripherals() {
    return new Promise<Peripheral[]>((fulfill, reject) => {
      witBleManager.getDiscoveredPeripherals(
        (error: string | null, result: Peripheral[] | null) => {
          if (error) {
            reject(error);
          } else {
  ...
  removePeripheral(peripheralId: string) {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.removePeripheral(peripheralId, (error: string | null) => {
        if (error) {
          reject(error);
        } else {
          fulfill();
  ...
  isScanning() {
    return new Promise<boolean>((fulfill, reject) => {
      witBleManager.isScanning((error: string | null, status: boolean) => {
        if (error) {
          reject(error);
        } else {
          fulfill(status);
  ...
  requestConnectionPriority(
    ...
    return new Promise<boolean>((fulfill, reject) => {
      witBleManager.requestConnectionPriority(
        peripheralId,
        connectionPriority,
  ...
  getAssociatedPeripherals() {
    return new Promise<Peripheral[]>((fulfill, reject) => {
      witBleManager.getAssociatedPeripherals(
  ...
  removeAssociatedPeripheral(peripheralId: string) {
    return new Promise<void>((fulfill, reject) => {
      witBleManager.removeAssociatedPeripheral(
        peripheralId,
        (error: string | null) => {
          if (error !== null) {
            reject(error);
  ...
  supportsCompanion() {
    return new Promise<boolean>((fulfill) => {
      witBleManager.supportsCompanion((supports: boolean) => fulfill(supports));
    });
  }
  ...
  getMaximumWriteValueLengthForWithoutResponse(peripheralId: string) {
    return new Promise<number>((fulfill, reject) => {
      witBleManager.getMaximumWriteValueLengthForWithoutResponse(
        peripheralId,
  ...
  getMaximumWriteValueLengthForWithResponse(peripheralId: string) {
    return new Promise<number>((fulfill, reject) => {
      witBleManager.getMaximumWriteValueLengthForWithResponse(
        peripheralId,
        (error: string | null, max: number) => {
          if (error) {
            reject(error);
...
export default new WitBleManager();
