...
const PeripheralDetailsScreen = ({route}: PeripheralDetailsProps) => {
  ...
  useEffect(() => {
    getDataInterval.current = setInterval(async () => {
      try {
        setData(await route.params.manager.getData(peripheralData.id));
      } catch (error) {
  ...
  return (
    <ScrollView
      ...
      {dataKeys.map((deviceDataKey, index) => (
        <View key={index} style={styles.serviceContainer}>
          <Text style={styles.serviceTitle}>
            {deviceDataKey}: {data[deviceDataKey]}
          </Text>
        </View>
      ))}
    </ScrollView>
