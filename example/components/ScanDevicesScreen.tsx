...
import WitBleManager, {
  ...
  PeripheralInfo,
  DeviceData,
} from 'react-native-ble-wit';

const SECONDS_TO_SCAN_FOR = 3;
const SERVICE_UUIDS: string[] = [];
const ALLOW_DUPLICATES = true;

const WitBleManagerModule = NativeModules.WitBleManager;
const bleManagerEmitter = new NativeEventEmitter(WitBleManagerModule);

declare module 'react-native-ble-wit' {
...
const ScanDevicesScreen = () => {
  ...
  const startCompanionScan = () => {
    ...
    try {
      ...
      WitBleManager.companionScan(SERVICE_UUIDS, {single: false})
  ...
  const enableBluetooth = async () => {
    try {
      ...
      await WitBleManager.enableBluetooth();
    } catch (error) {
  ...
  const togglePeripheralConnection = async (peripheral: Peripheral) => {
    if (peripheral && peripheral.connected) {
      try {
        await WitBleManager.disconnect(peripheral.id);
      } catch (error) {
        console.error(
  ...
  const retrieveConnected = async () => {
    try {
      const connectedPeripherals =
        await WitBleManager.getConnectedPeripherals();
    ...
    for (let [peripheralId, peripheral] of peripherals) {
      if (peripheral.connected) {
        const newPeripheralInfo =
          await WitBleManager.retrieveServices(peripheralId);
  ...
  const readCharacteristics = async () => {
    ...
    for (let peripheralInfo of services) {
      peripheralInfo.characteristics?.forEach(async c => {
        try {
          const value = await WitBleManager.read(
            peripheralInfo.id,
            c.service,
            c.characteristic,
          );
  ...
  const getAssociatedPeripherals = async () => {
    try {
      const associatedPeripherals =
        await WitBleManager.getAssociatedPeripherals();
      console.debug(
  ...
  const connectPeripheral = async (peripheral: Peripheral) => {
    try {
      if (peripheral) {
        ...
        await WitBleManager.connect(peripheral.id);
        ...
        const peripheralData = await WitBleManager.retrieveServices(
          peripheral.id,
        );
        const deviceData = await WitBleManager.getData(peripheral.id);
        ...
        const rssi = await WitBleManager.readRSSI(peripheral.id);
        console.debug(
          ...
          for (let characteristic of peripheralData.characteristics) {
            if (characteristic.descriptors) {
              for (let descriptor of characteristic.descriptors) {
                try {
                  let data = await WitBleManager.readDescriptor(
                    peripheral.id,
  ...
  useEffect(() => {
    try {
      WitBleManager.start({showAlert: false})
        .then(() => console.debug('WitBleManager started.'))
        .catch((error: any) =>
          console.error('BeManager could not be started.', error),
        );
    } catch (error) {
      console.error('unexpected error starting WitBleManager.', error);
      return;
    }

    const listeners = [
      bleManagerEmitter.addListener(
        'WitBleManagerDiscoverPeripheral',
        handleDiscoverPeripheral,
      ),
      bleManagerEmitter.addListener('WitBleManagerStopScan', handleStopScan),
      bleManagerEmitter.addListener(
